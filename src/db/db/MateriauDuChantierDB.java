...
public class MateriauDuChantierDB {
    ...
    public static List<MateriauDuChantierDto> getCollection(MateriauDuChantierSel sel) throws DevisChantierDbException {
        ...
        try {
            String query = "Select id_materiauDuChantier, categorie, tonnage, capacite, location, marque, modele, numerodechassis, carburant, prixhtva, ctamortmois FROM MateriauDuChantier ";
            java.sql.Connection connexion = DBManager.getConnection();
            java.sql.PreparedStatement stmt;
            String where = "";
            /*Pour une valeur numerique */
            if (sel.getIdMateriauDuChantier() != 0) {
                where = where + " id_materiauDuChantier = ? ";
            }
    ...
    public static int insertDb(MateriauDuChantierDto el) throws DevisChantierDbException {
        try {
            ...
            insert = connexion.prepareStatement(
                    "Insert into MateriauDuChantier(idMateriauDuChantier, categorie, tonnage, capacite, location, marque, modele, numeroChassis, carburant, prixHtva, ctAmortMois) "
                    + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
